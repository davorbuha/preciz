...
import { getRegistracijaById, getPrikolicaRegistracijaById, getVozacById } from '../../components/helpers';
...
export async function saveImageToFile(imageSource: string, uuid: string) {
	return new Promise<void>(resolve =>
		fs.writeFile(`${uuid}.jpeg`, imageSource.replace('data:image/jpeg;base64,', ''), 'base64', function(err) {
			console.log(err, 'err');
			resolve();
		})
	);
...
function JednoVaganjeScreen(props: Props) {
	...
	const tezinaVozilaInt = isNaN(parseInt(tezinaVozila)) ? 0 : parseInt(tezinaVozila);
	const tezinaPrikoliceInt = isNaN(parseInt(tezinaPrikolice)) ? 0 : parseInt(tezinaPrikolice);
	const onVoziloChange = React.useCallback(
		ev => {
			(voziloMasaRef.current as any).setTitle(props.vozila.find(item => item.id === ev[0])!.tezina);
	...
	const onPrikolicaChange = React.useCallback(
		ev => {
			...
			setTezinaPrikolice(props.prikolice.find(item => item.id === ev[0])!.tezinaPrikolice);
			return ev[0];
		},
		[props.prikolice]
	);
	...
	const handleSpremiPress = () => {
		props.handleSubmit(async values => {
			...
			const prikolica = prikolicaId
				? (await getPrikolicaRegistracijaById(prikolicaId)).registracijaPrikolice
				: '';
