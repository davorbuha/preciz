...
import { saveImageToFile, readImageFromFile } from './JednoVaganjeScreen';
...
function DrugoVaganjeScreen(props: Props) {
	...
	const bruto = selected ? (selected?.tip === 'Ulaz' ? selected.bruto : vrijednostVageInt) : 0;
	const tara = selected ? (selected.tip === 'Ulaz' ? vrijednostVageInt : selected.bruto) : 0;
	const neto = selected ? bruto - tara : 0;
	const handleSpremiPress = async () => {
		const ts = moment();
		const id = uuidv4();
		await saveImageToFile(imageSource, id);
		const firstImage = await readImageFromFile(selected!.id);
					...
					<TableBody>
						{filteredPrvaVaganje.map(item => (
							<TableRow
								...
								style={{
									backgroundColor: item.id === selected?.id ? colors.primary : undefined,
								}}
								key={item.id}>
								...
								<TableCell align="left">{item.ts.format('DD.MM.YYYY HH:mm')}</TableCell>
